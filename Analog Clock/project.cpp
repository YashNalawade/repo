...
void main()
{
 ...
 {
  setcolor(WHITE);
  outtextxy((getmaxx()/2)-75,20,"***ANALOG CLOCK***");
  clockLayout();
  secHand();
  minHand();
  hrHand();
  setcolor(LIGHTRED);
  outtextxy(30,getmaxy()-50,"Project made by :-");
  setcolor(LIGHTBLUE);
  outtextxy(30,getmaxy()-38,"Yash");
  outtextxy(30,getmaxy()-26,"S.E.(Comp)");
...
{
 struct time t;
 int r=100,x=maxx/2,y=maxy/2,sec;
 float O;
